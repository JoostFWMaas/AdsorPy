...
CoordPair: TypeAlias = np.ndarray[tuple[Literal[2], Literal[1]], np.dtype[np.double]]  # 2x1 array of coordinates
"""2x1 array of a single coordinate pair."""
CoordsArray: TypeAlias = np.ndarray[tuple[Literal[2], int], np.dtype[np.double]]  # 2xN array of coords.
"""2xn array of coordinates."""
CoordsArray3D = np.ndarray[tuple[Literal[3], int], np.dtype[np.double]]  # 2 or 3 x N array of coords.
"""3xn array of floats. 3D coordinates."""
DistArray: TypeAlias = np.ndarray[tuple[int], np.dtype[np.double]]
"""1D array of floats. Often used to denote distances, so usually positive."""
FloatArray: TypeAlias = NDArray[np.double]
...
IdxArray: TypeAlias = np.ndarray[tuple[int], np.dtype[np.long]]  # Flat index aray of integers.
...
RotMatrix: TypeAlias = np.ndarray[tuple[Literal[3], Literal[3]], np.dtype[np.double]]
